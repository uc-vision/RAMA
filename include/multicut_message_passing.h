...
inline VectorType<int> invert_unique_impl(const VectorType<int>& values, const VectorType<int>& counts) {
    ...
    thrust::scatter(RAMA_THRUST_EXEC thrust::constant_iterator<int>(1), thrust::constant_iterator<int>(1) + values.size(),
    ...
    thrust::transform(RAMA_THRUST_EXEC output_indices.begin(), output_indices.end(),
    ...
    VectorType<int>& edge_counter, VectorType<int>& triangle_correspondence_ab)
{
    ...
    thrust::sequence(RAMA_THRUST_EXEC t_sort_order.begin(), t_sort_order.end());
    ...
    VectorType<int> t_counts(ta.size());
    {
        ...
        thrust::sort_by_key(RAMA_THRUST_EXEC first, last, t_sort_order.begin());

        auto first_unique = thrust::make_zip_iterator(thrust::make_tuple(ta_unique.begin(), tb_unique.begin()));
        auto last_reduce = thrust::reduce_by_key(RAMA_THRUST_EXEC first, last, thrust::make_constant_iterator(1), first_unique, t_counts.begin());
    ...
    assert(thrust::is_sorted(RAMA_THRUST_EXEC first_edge, last_edge));
    assert(std::distance(first_edge, thrust::unique(RAMA_THRUST_EXEC first_edge, last_edge)) == i.size());
    ...
    thrust::scatter(RAMA_THRUST_EXEC correspondence_sorted.begin(), correspondence_sorted.end(), t_sort_order.begin(), triangle_correspondence_ab.begin());

    VectorType<int> edge_increment(i.size(), 0);
    thrust::scatter(RAMA_THRUST_EXEC t_counts.begin(), t_counts.end(), unique_correspondence.begin(), edge_increment.begin());
    thrust::transform(RAMA_THRUST_EXEC edge_counter.begin(), edge_counter.end(), edge_increment.begin(), edge_counter.begin(), thrust::plus<int>());
}
...
multicut_message_passing<VectorType>::extract_directed_edges(const Graph<VectorType>& A)
{
    ...
    size_t n_dir = thrust::count_if(RAMA_THRUST_EXEC begin, end, is_directed);
    ...
    thrust::copy_if(RAMA_THRUST_EXEC begin, end, out_begin, is_directed);
        ...
        VectorType<int>&& new_t1, VectorType<int>&& new_t2, VectorType<int>&& new_t3)
{
    ...
    //    edges that may not exist in the original graph.
    {
        ...
        thrust::copy(RAMA_THRUST_EXEC new_t1.begin(), new_t1.end(), tri_i.begin());
        thrust::copy(RAMA_THRUST_EXEC new_t2.begin(), new_t2.end(), tri_j.begin());
        thrust::copy(RAMA_THRUST_EXEC new_t1.begin(), new_t1.end(), tri_i.begin() + num_new);
        thrust::copy(RAMA_THRUST_EXEC new_t3.begin(), new_t3.end(), tri_j.begin() + num_new);
        thrust::copy(RAMA_THRUST_EXEC new_t2.begin(), new_t2.end(), tri_i.begin() + 2 * num_new);
        thrust::copy(RAMA_THRUST_EXEC new_t3.begin(), new_t3.end(), tri_j.begin() + 2 * num_new);
    ...
    thrust::copy(RAMA_THRUST_EXEC new_t1.begin(), new_t1.end(), t1.begin() + old_size);
    thrust::copy(RAMA_THRUST_EXEC new_t2.begin(), new_t2.end(), t2.begin() + old_size);
    thrust::copy(RAMA_THRUST_EXEC new_t3.begin(), new_t3.end(), t3.begin() + old_size);
    ...
    // 4. Sort combined triangles by (t1,t2,t3), carrying cost arrays via permutation
    {
        ...
        thrust::sequence(RAMA_THRUST_EXEC perm.begin(), perm.end());
        ...
        thrust::sort_by_key(RAMA_THRUST_EXEC key_first, key_last, perm.begin());

        VectorType<float> tmp12(total_size), tmp13(total_size), tmp23(total_size);
        thrust::gather(RAMA_THRUST_EXEC perm.begin(), perm.end(), t12_costs.begin(), tmp12.begin());
        thrust::gather(RAMA_THRUST_EXEC perm.begin(), perm.end(), t13_costs.begin(), tmp13.begin());
        thrust::gather(RAMA_THRUST_EXEC perm.begin(), perm.end(), t23_costs.begin(), tmp23.begin());
...
{
    return thrust::transform_reduce(RAMA_THRUST_EXEC edge_costs.begin(), edge_costs.end(),
...
double multicut_message_passing<VectorType>::triangle_lower_bound()
{
    ...
    return thrust::transform_reduce(RAMA_THRUST_EXEC first, last,
...
void multicut_message_passing<VectorType>::send_messages_to_triplets()
{
    ...
    // send costs to triangles
    {
        ...
        thrust::for_each(RAMA_THRUST_EXEC first, last, increase_triangle_costs);
    }
    {
    ...
    // set costs of edges to zero (if edge participates in a triangle)
    {
        ...
        thrust::for_each(RAMA_THRUST_EXEC first, last,
...
void multicut_message_passing<VectorType>::send_messages_to_edges()
{
    ...
    VectorType<float> d12(num_triangles), d13(num_triangles), d23(num_triangles);
    {
        ...
        thrust::for_each(RAMA_THRUST_EXEC thrust::make_counting_iterator(0),
    ...
    thrust::copy(RAMA_THRUST_EXEC triangle_correspondence_12.begin(), triangle_correspondence_12.end(), all_idx.begin());
    thrust::copy(RAMA_THRUST_EXEC triangle_correspondence_13.begin(), triangle_correspondence_13.end(), all_idx.begin() + num_triangles);
    thrust::copy(RAMA_THRUST_EXEC triangle_correspondence_23.begin(), triangle_correspondence_23.end(), all_idx.begin() + 2 * num_triangles);
    thrust::copy(RAMA_THRUST_EXEC d12.begin(), d12.end(), all_deltas.begin());
    thrust::copy(RAMA_THRUST_EXEC d13.begin(), d13.end(), all_deltas.begin() + num_triangles);
    thrust::copy(RAMA_THRUST_EXEC d23.begin(), d23.end(), all_deltas.begin() + 2 * num_triangles);

    // Phase 3: sort by edge index so reduce_by_key can aggregate per edge.
    thrust::sort_by_key(RAMA_THRUST_EXEC all_idx.begin(), all_idx.end(), all_deltas.begin());
    ...
    auto reduce_end = thrust::reduce_by_key(RAMA_THRUST_EXEC 
    ...
    thrust::scatter(RAMA_THRUST_EXEC reduced_vals.begin(), reduced_vals.begin() + num_unique,
                    reduced_keys.begin(), increments.begin());
    thrust::transform(RAMA_THRUST_EXEC edge_costs.begin(), edge_costs.end(), increments.begin(),
